...
def create_tensorflow_graph(layer_list, num_training_examples):
    ...
    for j in range(1, len(layer_list)):
        W = tf.get_variable("W" + str(j), shape = [layer_list[j], layer_list[j-1]], dtype = tf.float32, initializer = tf.glorot_normal_initializer())
        B = tf.get_variable("B" + str(j), shape = [layer_list[j], 1], dtype = tf.float32, initializer = tf.glorot_normal_initializer())
...
def test_backprop_with_tensorflow():
    ...
    with tf.Session() as sess:
        ...
        for i in range(num_iterations):
            ...
            train_op_value, cost_value, yhat_value = sess.run([train_op, cost, yhat], feed_dict={X_node: X_val, Y_node: Y_val})
